...
<div class="card-header d-flex justify-content-between align-items-center">
    <h3 class="mb-0"><strong>Forwarded Survey List</strong></h3>

<div class="d-flex align-items-center mb-0">
    <label for="session_date" class="mr-2 mb-0"><strong>Select Session Date:</strong></label>

    <form id="sessionForm" method="GET" class="mb-0">
        <select class="form-control" id="session_date" name="session_date"
                onchange="document.getElementById('sessionForm').submit();"
                style="min-width: 200px;">
            @foreach ($last_five_session_dates as $date)
                <option value="{{ $date }}" {{ $selected_session_date == $date ? 'selected' : '' }}>
                    {{ $date }}
                </option>
            @endforeach
        </select>
